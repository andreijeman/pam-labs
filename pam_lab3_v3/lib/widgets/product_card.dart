    ...
    final widgetImg = img.startsWith('http')
        ? Image.network(
            ...
            loadingBuilder: (c, child, progress) =>
                progress == null ? child : const SizedBox(height: 160, child: Center(child: CircularProgressIndicator(strokeWidth: 2))),
            errorBuilder: (c, e, s) => const SizedBox(
              height: 160,
              child: Center(child: Icon(Icons.broken_image_outlined)),
            ),
          )
        : Image.asset(img, height: 160, width: double.infinity, fit: BoxFit.cover);
    return widgetImg;
  }

  @override
  Widget build(BuildContext context) {
    // Pick a price safely (supports "Sale" items that only have newPrice)
    final double effectivePrice = (product.price ?? product.newPrice ?? 0).toDouble();

    // Show old price only if present and bigger than current
    final double? oldPrice =
        (product.oldPrice != null && product.oldPrice! > effectivePrice) ? product.oldPrice : null;

    // Discount badge: prefer given % else compute from old/new
    int? discountPercent = product.discount;
    if (discountPercent == null && oldPrice != null && oldPrice > 0 && effectivePrice > 0) {
      discountPercent = (((oldPrice - effectivePrice) / oldPrice) * 100).round();
      ...
      child: SizedBox(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              product.name, // ← our model uses `name` from JSON
              maxLines: 1,
